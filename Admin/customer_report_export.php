<?php
    session_start();
    include("config.php");
    $cus_id = $_SESSION['cus_id'];
    $total = 0;
    if($cus_id == 0){
        $s = 1;
        $sql = "SELECT * FROM tbl_category c INNER JOIN tbl_item i ON i.category_id=c.category_id INNER JOIN tbl_cart_item ci ON ci.item_id=i.item_id INNER JOIN tbl_transaction t ON t.cart_item_id=ci.cart_item_id WHERE t.transaction_status='1' AND t.wallet_id!='0'";
        $res = mysqli_query($con,$sql);
        $html = '<table border="1"><tr><th style="background:darkblue;color:#fff;">Sl. no.</th><th style="background:darkblue;color:#fff;">Item Name</th style="background:darkblue;color:#fff;"><th style="background:darkblue;color:#fff;">Date of Purchase</th><th style="background:darkblue;color:#fff;">Income</th></tr>';
        while($row = mysqli_fetch_assoc($res)){
            $amount = $row['transaction_amount']/97*100;
            $total = $total + $row['transaction_amount'];
            $html.='<tr><td>'.$s++.'</td><td>'.$row['item_name'].'</td><td>'.$row['transaction_date'].'</td><td>'.$amount.'</td></tr>';
        }
        $html.='<tr><th colspan="3" style="background:darkblue;color:#fff;">Total Income</th><th style="background:darkblue;color:#fff;">'.$total.'</th></tr></table>';
        header('Content-Type:applications/xls');
        header('Content-Disposition:attachment;filename=customer_report.xls');
        echo $html;
    } else {
        $s = 1;
        $sql = "SELECT * FROM tbl_category c INNER JOIN tbl_item i ON i.category_id=c.category_id INNER JOIN tbl_cart_item ci ON ci.item_id=i.item_id INNER JOIN tbl_transaction t ON t.cart_item_id=ci.cart_item_id INNER JOIN tbl_customer cu ON cu.customer_id=ci.cart_id WHERE t.transaction_status='1' AND t.wallet_id!='0' AND cu.customer_id='$cus_id'";
        $res = mysqli_query($con,$sql);
        $html = '<table border="1"><tr><th style="background:darkblue;color:#fff;">Sl. no.</th><th style="background:darkblue;color:#fff;">Item Name</th style="background:darkblue;color:#fff;"><th style="background:darkblue;color:#fff;">Date of Purchase</th><th style="background:darkblue;color:#fff;">Income</th></tr>';
        while($row = mysqli_fetch_assoc($res)){
            $amount = $row['transaction_amount']/97*100;
            $total = $total + $row['transaction_amount'];
            $html.='<tr><td>'.$s++.'</td><td>'.$row['item_name'].'</td><td>'.$row['transaction_date'].'</td><td>'.$amount.'</td></tr>';
        }
        $html.='<tr><th colspan="3" style="background:darkblue;color:#fff;">Total Income</th><th style="background:darkblue;color:#fff;">'.$total.'</th></tr></table>';
        header('Content-Type:applications/xls');
        header('Content-Disposition:attachment;filename=customer_report.xls');
        echo $html;
    }
?>